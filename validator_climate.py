...
Colonnes obligatoires (après normalisation) : Name, Data Type, Location/Climate
Les alias sont acceptés et renommés automatiquement.
"""
...
# ==============================================================================
# 2. COLONNES OBLIGATOIRES ET LEURS ALIAS
# ==============================================================================
MANDATORY_FIELDS = ("Name", "Data Type", "Location/Climate")

# Alias pour la normalisation (les clés sont les noms canoniques)
MANDATORY_FIELD_ALIASES = {
    "Name": ["Name", "name", "nom", "station_name"],
    "Data Type": ["Data Type", "data_type", "DataType", "data type", "datatype"],
    "Location/Climate": ["Location/Climate", "Location", "Climate", "location_climate", "locationclimate"],
}

# Types de données autorisés pour "Data Type" (avec variantes)
ALLOWED_DATA_TYPE_VARIATIONS = {
    "numeric": "numeric",
    "numérique": "numeric",
    "num": "numeric",
    "date": "date",
...
ALLOWED_DATA_TYPES = set(ALLOWED_DATA_TYPE_VARIATIONS.values())

# ==============================================================================
# 3. DÉTECTION DES COLONNES CLIMATIQUES (protège les colonnes obligatoires)
# ==============================================================================
CLIMATE_KEYWORDS = {
    "station_id":   ["station", "site", "code"],
    ...
    "altitude":     ["altitude", "elevation", "alt"],
...
# ==============================================================================
# 4. NORMALISATION DES COLONNES OBLIGATOIRES
# ==============================================================================
def _normalize_mandatory_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Renomme les colonnes correspondant aux alias vers les noms canoniques.
    Les alias sont définis dans MANDATORY_FIELD_ALIASES.
    """
    df = df.copy()
    alias_to_canonical = {}
    for canon, aliases in MANDATORY_FIELD_ALIASES.items():
        for alias in aliases:
            alias_to_canonical[alias.lower().strip()] = canon

    for col in df.columns:
        col_lower = str(col).lower().strip()
        if col_lower in alias_to_canonical:
            new_name = alias_to_canonical[col_lower]
            # Éviter les conflits si le nom canonique existe déjà
            if new_name in df.columns and df.columns.get_loc(new_name) != df.columns.get_loc(col):
                print(f"[WARN] Colonne '{col}' mapperait vers '{new_name}' mais existe déjà. Ignoré.", file=sys.stderr)
            else:
                df.rename(columns={col: new_name}, inplace=True)
    return df

# ==============================================================================
# 5. MAPPING DES COLONNES CLIMATIQUES (ne touche pas aux colonnes obligatoires)
# ==============================================================================
def _map_columns(df: pd.DataFrame) -> dict:
    """Retourne {nom_normalisé: nom_original} pour les colonnes climatiques,
       en ignorant les colonnes obligatoires (nom canonique)."""
    mapping = {}
    df_cols_lower = {str(c).strip().lower(): c for c in df.columns}
    # Colonnes à ignorer : les noms canoniques
    mandatory_canonical = set(MANDATORY_FIELDS)
    for standard, patterns in CLIMATE_KEYWORDS.items():
        for p in patterns:
            p_low = p.lower()
            for col_lower, orig in df_cols_lower.items():
                # Ne pas mapper si la colonne est une colonne obligatoire (nom canonique)
                if (p_low in col_lower or col_lower in p_low) and orig not in mandatory_canonical:
...
def _standardize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Applique le mapping et convertit les types pour les colonnes reconnues."""
...
def _load_multi_sheet(excel_path: str) -> tuple[pd.DataFrame, str]:
    """Charge toutes les feuilles, normalise les colonnes obligatoires, fusionne et retourne le DataFrame fusionné."""
    ...
    for sheet_name in xl.sheet_names:
        ...
        if not df.empty:
            # Étape 1 : normaliser les colonnes obligatoires
            df = _normalize_mandatory_columns(df)
    ...
    # Étape 2 : standardiser les colonnes climatiques (ne touche pas aux obligatoires)
...
# ==============================================================================
# 7. SCHÉMA FRICTIONLESS (pour les colonnes climatiques)
# ==============================================================================
...
def _check_data_type_values(df: pd.DataFrame) -> list[dict]:
    """Vérifie que les valeurs de la colonne 'Data Type' sont valides."""
    issues = []
    if "Data Type" not in df.columns:
        return issues
    for idx, row in df.iterrows():
        val = row.get("Data Type")
        if pd.isna(val):
            issues.append({
                "row": idx+2,
                "accenumb": row.get("Name", f"ligne{idx+2}"),
                "category": "INVALID_DATA_TYPE",
                "field": "Data Type",
                "level": "ERROR",
                "message": "Valeur manquante pour le type de données."
            })
            continue
        val_str = str(val).strip().lower()
        canonical = ALLOWED_DATA_TYPE_VARIATIONS.get(val_str)
        if canonical is None and val_str in ALLOWED_DATA_TYPES:
            canonical = val_str
        if canonical is None:
            issues.append({
                ...
                "field": "Data Type",
                "level": "ERROR",
                "message": f"Type de données non reconnu : '{val}'. Attendu : numeric, date, text, categorical (ou variantes)."
            })
    return issues
...
def _format_report(excel_path: str, df: pd.DataFrame, frictionless_report, extra_issues: list[dict]) -> str:
    ...
    lines.append("  * Data Type values  : numeric, date, text, categorical (variantes acceptées)")
...
def run_validation(excel_path: str) -> str:
    ...
    # Vérifications métier (incluant les colonnes obligatoires)
    extra = []
    extra.extend(_check_mandatory_fields(df))
    extra.extend(_check_data_type_values(df))
